(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("f44e30c2-b652-41ed-8e07-5112b9a493a0");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid 'f44e30c2-b652-41ed-8e07-5112b9a493a0' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error() {
          console.error("failed to load " + url);
        }
    
        for (var i = 0; i < css_urls.length; i++) {
          var url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error;
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.2.3.min.js": "T2yuo9Oe71Cz/I4X9Ac5+gpEa5a8PpJCDlqKYO0CfAuEszu1JrXLl8YugMqYe3sM", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.3.min.js": "98GDGJ0kOMCUMUePhksaQ/GYgB3+NH9h996V88sh3aOiUNX3N+fLXAtry6xctSZ6", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.3.min.js": "89bArO+nlbP3sgakeHjCo1JYxYR5wufVgA3IbUvDY+K7w4zyxJqssu7wVnfeKCq8"};
    
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error;
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.2.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.3.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"069d9054-adc1-435e-a980-ad9a106cc9c1":{"roots":{"references":[{"attributes":{"children":[{"id":"14817"},{"id":"14815"}]},"id":"14818","type":"Column"},{"attributes":{"source":{"id":"14775"}},"id":"14779","type":"CDSView"},{"attributes":{"source":{"id":"14770"}},"id":"14774","type":"CDSView"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"14786"},"selection_policy":{"id":"14787"}},"id":"14770","type":"ColumnDataSource"},{"attributes":{},"id":"14757","type":"SaveTool"},{"attributes":{},"id":"14811","type":"UnionRenderers"},{"attributes":{"source":{"id":"14794"}},"id":"14798","type":"CDSView"},{"attributes":{"axis":{"id":"14743"},"ticker":null},"id":"14746","type":"Grid"},{"attributes":{"toolbars":[{"id":"14761"}],"tools":[{"id":"14751"},{"id":"14752"},{"id":"14753"},{"id":"14754"},{"id":"14755"},{"id":"14756"},{"id":"14757"},{"id":"14758"}]},"id":"14816","type":"ProxyToolbar"},{"attributes":{},"id":"14786","type":"Selection"},{"attributes":{},"id":"14810","type":"Selection"},{"attributes":{"data":{},"selected":{"id":"14810"},"selection_policy":{"id":"14811"}},"id":"14794","type":"ColumnDataSource"},{"attributes":{"below":[{"id":"14743"}],"center":[{"id":"14746"},{"id":"14750"},{"id":"14792"}],"left":[{"id":"14747"}],"output_backend":"webgl","plot_height":500,"plot_width":500,"renderers":[{"id":"14773"},{"id":"14778"},{"id":"14797"}],"title":{"id":"14799"},"toolbar":{"id":"14761"},"toolbar_location":null,"x_range":{"id":"14735"},"x_scale":{"id":"14739"},"y_range":{"id":"14737"},"y_scale":{"id":"14741"}},"id":"14734","subtype":"Figure","type":"Plot"},{"attributes":{"callback":null},"id":"14758","type":"HoverTool"},{"attributes":{},"id":"14789","type":"UnionRenderers"},{"attributes":{},"id":"14741","type":"LinearScale"},{"attributes":{},"id":"14748","type":"BasicTicker"},{"attributes":{"axis":{"id":"14747"},"dimension":1,"ticker":null},"id":"14750","type":"Grid"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"14778"}]},"id":"14793","type":"LegendItem"},{"attributes":{},"id":"14756","type":"UndoTool"},{"attributes":{},"id":"14739","type":"LinearScale"},{"attributes":{"overlay":{"id":"14759"}},"id":"14753","type":"BoxZoomTool"},{"attributes":{},"id":"14751","type":"ResetTool"},{"attributes":{"data":{},"selected":{"id":"14788"},"selection_policy":{"id":"14789"}},"id":"14775","type":"ColumnDataSource"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"xs_units":"screen","ys_units":"screen"},"id":"14760","type":"PolyAnnotation"},{"attributes":{"children":[[{"id":"14734"},0,0]]},"id":"14815","type":"GridBox"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","top_units":"screen"},"id":"14759","type":"BoxAnnotation"},{"attributes":{},"id":"14782","type":"BasicTickFormatter"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"units":"screen","value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"14795","type":"Circle"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14772","type":"Line"},{"attributes":{},"id":"14752","type":"PanTool"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14776","type":"Line"},{"attributes":{"text":"y","text_font_size":{"value":"15pt"}},"id":"14799","type":"Title"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"14751"},{"id":"14752"},{"id":"14753"},{"id":"14754"},{"id":"14755"},{"id":"14756"},{"id":"14757"},{"id":"14758"}]},"id":"14761","type":"Toolbar"},{"attributes":{},"id":"14784","type":"BasicTickFormatter"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14771","type":"Line"},{"attributes":{"overlay":{"id":"14760"}},"id":"14755","type":"LassoSelectTool"},{"attributes":{},"id":"14754","type":"WheelZoomTool"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"line_alpha":{"value":0.1},"size":{"units":"screen","value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"14796","type":"Circle"},{"attributes":{},"id":"14787","type":"UnionRenderers"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14777","type":"Line"},{"attributes":{},"id":"14737","type":"DataRange1d"},{"attributes":{"data_source":{"id":"14770"},"glyph":{"id":"14771"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14772"},"selection_glyph":null,"view":{"id":"14774"}},"id":"14773","type":"GlyphRenderer"},{"attributes":{},"id":"14744","type":"BasicTicker"},{"attributes":{"data_source":{"id":"14794"},"glyph":{"id":"14795"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14796"},"selection_glyph":null,"view":{"id":"14798"}},"id":"14797","type":"GlyphRenderer"},{"attributes":{"items":[{"id":"14793"}]},"id":"14792","type":"Legend"},{"attributes":{"formatter":{"id":"14782"},"ticker":{"id":"14744"}},"id":"14743","type":"LinearAxis"},{"attributes":{"formatter":{"id":"14784"},"ticker":{"id":"14748"}},"id":"14747","type":"LinearAxis"},{"attributes":{},"id":"14735","type":"DataRange1d"},{"attributes":{},"id":"14788","type":"Selection"},{"attributes":{"data_source":{"id":"14775"},"glyph":{"id":"14776"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14777"},"selection_glyph":null,"view":{"id":"14779"}},"id":"14778","type":"GlyphRenderer"},{"attributes":{"toolbar":{"id":"14816"},"toolbar_location":"above"},"id":"14817","type":"ToolbarBox"}],"root_ids":["14818"]},"title":"Bokeh Application","version":"2.2.3"}}';
                  var render_items = [{"docid":"069d9054-adc1-435e-a980-ad9a106cc9c1","root_ids":["14818"],"roots":{"14818":"f44e30c2-b652-41ed-8e07-5112b9a493a0"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();