(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("40567ea5-1690-4be8-95e9-4b45c429be54");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '40567ea5-1690-4be8-95e9-4b45c429be54' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error() {
          console.error("failed to load " + url);
        }
    
        for (var i = 0; i < css_urls.length; i++) {
          var url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error;
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.2.3.min.js": "T2yuo9Oe71Cz/I4X9Ac5+gpEa5a8PpJCDlqKYO0CfAuEszu1JrXLl8YugMqYe3sM", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.3.min.js": "98GDGJ0kOMCUMUePhksaQ/GYgB3+NH9h996V88sh3aOiUNX3N+fLXAtry6xctSZ6", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.3.min.js": "89bArO+nlbP3sgakeHjCo1JYxYR5wufVgA3IbUvDY+K7w4zyxJqssu7wVnfeKCq8"};
    
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error;
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.2.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.3.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"0dbf4052-2a80-4123-942f-07e4f61d3497":{"roots":{"references":[{"attributes":{"source":{"id":"14657"}},"id":"14661","type":"CDSView"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"14671"},"selection_policy":{"id":"14672"}},"id":"14652","type":"ColumnDataSource"},{"attributes":{"overlay":{"id":"14636"}},"id":"14631","type":"LassoSelectTool"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","top_units":"screen"},"id":"14635","type":"BoxAnnotation"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"14653","type":"Line"},{"attributes":{"data_source":{"id":"14652"},"glyph":{"id":"14653"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14654"},"selection_glyph":null,"view":{"id":"14656"}},"id":"14655","type":"GlyphRenderer"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"14654","type":"Line"},{"attributes":{"overlay":{"id":"14635"}},"id":"14629","type":"BoxZoomTool"},{"attributes":{"text":"y","text_font_size":{"value":"15pt"}},"id":"14662","type":"Title"},{"attributes":{},"id":"14615","type":"LinearScale"},{"attributes":{"below":[{"id":"14619"}],"center":[{"id":"14622"},{"id":"14626"},{"id":"14651"}],"left":[{"id":"14623"}],"output_backend":"webgl","plot_height":500,"plot_width":500,"renderers":[{"id":"14649"},{"id":"14655"},{"id":"14660"}],"title":{"id":"14662"},"toolbar":{"id":"14637"},"toolbar_location":null,"x_range":{"id":"14611"},"x_scale":{"id":"14615"},"y_range":{"id":"14613"},"y_scale":{"id":"14617"}},"id":"14610","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"14632","type":"UndoTool"},{"attributes":{},"id":"14671","type":"Selection"},{"attributes":{},"id":"14633","type":"SaveTool"},{"attributes":{"source":{"id":"14652"}},"id":"14656","type":"CDSView"},{"attributes":{},"id":"14672","type":"UnionRenderers"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14659","type":"Line"},{"attributes":{},"id":"14630","type":"WheelZoomTool"},{"attributes":{"children":[{"id":"14680"},{"id":"14678"}]},"id":"14681","type":"Column"},{"attributes":{"formatter":{"id":"14667"},"ticker":{"id":"14624"}},"id":"14623","type":"LinearAxis"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"14669"},"selection_policy":{"id":"14670"}},"id":"14646","type":"ColumnDataSource"},{"attributes":{"formatter":{"id":"14665"},"ticker":{"id":"14620"}},"id":"14619","type":"LinearAxis"},{"attributes":{},"id":"14628","type":"PanTool"},{"attributes":{},"id":"14665","type":"BasicTickFormatter"},{"attributes":{"source":{"id":"14646"}},"id":"14650","type":"CDSView"},{"attributes":{},"id":"14670","type":"UnionRenderers"},{"attributes":{"toolbars":[{"id":"14637"}],"tools":[{"id":"14627"},{"id":"14628"},{"id":"14629"},{"id":"14630"},{"id":"14631"},{"id":"14632"},{"id":"14633"},{"id":"14634"}]},"id":"14679","type":"ProxyToolbar"},{"attributes":{},"id":"14624","type":"BasicTicker"},{"attributes":{},"id":"14611","type":"DataRange1d"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"14627"},{"id":"14628"},{"id":"14629"},{"id":"14630"},{"id":"14631"},{"id":"14632"},{"id":"14633"},{"id":"14634"}]},"id":"14637","type":"Toolbar"},{"attributes":{"data":{},"selected":{"id":"14673"},"selection_policy":{"id":"14674"}},"id":"14657","type":"ColumnDataSource"},{"attributes":{"data_source":{"id":"14657"},"glyph":{"id":"14658"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14659"},"selection_glyph":null,"view":{"id":"14661"}},"id":"14660","type":"GlyphRenderer"},{"attributes":{"bottom":1.4596776592684024,"fill_alpha":0.1,"fill_color":"#2a2eec","top":0.6850827603275129},"id":"14651","type":"BoxAnnotation"},{"attributes":{"axis":{"id":"14623"},"dimension":1,"ticker":null},"id":"14626","type":"Grid"},{"attributes":{"data_source":{"id":"14646"},"glyph":{"id":"14647"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14648"},"selection_glyph":null,"view":{"id":"14650"}},"id":"14649","type":"GlyphRenderer"},{"attributes":{},"id":"14667","type":"BasicTickFormatter"},{"attributes":{},"id":"14617","type":"LinearScale"},{"attributes":{},"id":"14673","type":"Selection"},{"attributes":{},"id":"14669","type":"Selection"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14647","type":"Line"},{"attributes":{},"id":"14627","type":"ResetTool"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"xs_units":"screen","ys_units":"screen"},"id":"14636","type":"PolyAnnotation"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14648","type":"Line"},{"attributes":{"toolbar":{"id":"14679"},"toolbar_location":"above"},"id":"14680","type":"ToolbarBox"},{"attributes":{},"id":"14620","type":"BasicTicker"},{"attributes":{},"id":"14674","type":"UnionRenderers"},{"attributes":{"axis":{"id":"14619"},"ticker":null},"id":"14622","type":"Grid"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14658","type":"Line"},{"attributes":{"children":[[{"id":"14610"},0,0]]},"id":"14678","type":"GridBox"},{"attributes":{},"id":"14613","type":"DataRange1d"},{"attributes":{"callback":null},"id":"14634","type":"HoverTool"}],"root_ids":["14681"]},"title":"Bokeh Application","version":"2.2.3"}}';
                  var render_items = [{"docid":"0dbf4052-2a80-4123-942f-07e4f61d3497","root_ids":["14681"],"roots":{"14681":"40567ea5-1690-4be8-95e9-4b45c429be54"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();